...
"""
Feature engineering for Irish DAM price forecasting.
"""
...
def wind_power_proxy(wind_ms: pd.Series) -> pd.Series:
    """Convert wind speed to power proxy using simplified power curve."""
...
def build_feature_table(
    dam: pd.Series,
    load_fc: pd.Series,
    windsol_fc: pd.DataFrame = None,
    weather: pd.DataFrame = None,
) -> pd.DataFrame:
    """
    Build feature table for Irish DAM price forecasting.
    
    Args:
        dam: DAM prices series (index should be datetime)
        load_fc: Load forecast series
        windsol_fc: Wind/solar forecast DataFrame (optional)
        weather: Weather DataFrame (optional)
    ...
    if windsol_fc is not None and isinstance(windsol_fc, pd.DataFrame) and not windsol_fc.empty:
    ...
    if "wind_total_mw" in df.columns and "load_forecast_mw" in df.columns:
        df["wind_share"] = (
            df["wind_total_mw"] / df["load_forecast_mw"].clip(lower=1)
        ).clip(0, 1)
